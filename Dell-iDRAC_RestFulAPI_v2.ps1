...
function Generate-CSR
{
    ...
    if ( -not $Specific.SubjectDn.CN ) { $missing += "Common Name" }
    if ( -not $Specific.SubjectDn.OU ) { $missing += "Organizational Unit" }
    if ( -not $Specific.SubjectDn.O ) { $missing += "Organization" }
    if ( -not $Specific.SubjectDn.L ) { $missing += "City/Locality" }
    if ( -not $Specific.SubjectDn.ST ) { $missing += "State/Province" }
    if ( -not $Specific.SubjectDn.C ) { $missing += "Country" }
...
# Generate iDRAC CSR
    ...
    $org     = Sanitize-String "Organization" $Specific.SubjectDn.O
...
# Waiting loop for CSR generation; if required
    ...
    $LienGet  = "https://" + $addr +"/redfish/v1/CertificateService/"
...
<#
        # Timeouts between recovery attempts (If required) 
        Write-VenafiDebug -Message "Wait 15 seconds before the next attempt." 
